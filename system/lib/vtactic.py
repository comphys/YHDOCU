...
class Vtactic :
    ...
    def nextStep(self) :
        ...
        if  self.M['첫날기록'] : 
            ...
            self.D['N_일반매도가'] = 0.0
            self.D['N_일반종대비'] = self.next_percent(self.M['당일종가'],self.D['N_기회매수가'])
            self.D['N_안정종대비'] = ''
    ...
    def get_tacticLog(self,theDate) :
        ...
        if  not self.V['매수금액'] and not self.V['매도금액'] and LD['sub12'] : LD['sub29'] = '매도대기'
    ...
    def get_nextStrategyLog(self,tac) :
        ...
        nS = {'sub18':self.D['N_일반기초'],'sub2' :self.D['N_일반매수량'],'sub3':self.D['N_일반매도량'],
              'sub19':self.D['N_일반매수가'],'sub20':self.D['N_일반매도가']}
        return nS
