...
# ------------------------------------------------------------------------------------------------------------------------------------------ 
    ...
    def calculate(self)  :
        ...
        if  self.V['매도수량'] :
            ...
            self.L['매도금액'] = self.L['중도합계']
            self.L['수익현황'] = self.L['중익합계']
            self.L['현수익률'] = self.L['실현익률']
    ...
    def today_sell(self) :
        ...
        if  self.M['당일종가'] >= self.L['매도예가'] :  # N 매도예가는 M 매도예가과 같거나 작아야 한다.
            self.L['매도수량']  = self.L['보유수량']
    ...
    def today_buy_L(self) :
        
        if  self.M['당일종가'] <= self.L['매수예가'] and self.L['예정수량'] :
            self.L['매수수량']  = self.L['예정수량']
            self.L['매수차수']  = self.L['매수차수'] + 1 if self.L['매수차수'] < self.L['최대차수'] else self.L['최대차수']
            self.L['거래코드']  = f"{self.L['매수차수']}B {self.L['매수수량']}"
    ...
    def take_chanceL(self) :
        ...
        cp = round(A/(k-n),2)
        # R,S 의 매수가는 V의 매수가보다 낮아야 한다
        return round(min(cp, self.M['당일종가']*self.L['매입가치']),2)
    ...
    def tomorrow_buy_L(self) :
        ...
        self.L['매수예가'] = 0.0 
        self.L['매수예가'] = min( self.M['당일종가']*self.L['매입가치'], self.L['평균단가'] ) if self.L['보유수량'] else self.take_chanceL()
        self.L['매수예가'] = round(self.L['매수예가'],2)
    ...
    def tomorrow_step(self) :
        ...
        self.L['매도예가'] = min(self.M['매도예가'],self.L['매도예가'])
        
        for tac in (self.V,self.R,self.S,self.N,self.L) : 
    ...
    def simulate(self,printOut=False) :
        ...
        for idx,BD in enumerate(self.B) : 
            ...
            self.V['거래코드'] = self.R['거래코드'] = self.S['거래코드'] = self.N['거래코드'] = self.L['거래코드'] = ' '
    ...
    def result(self) :
        ...
        초기자본5 = self.D['럭키자금']; 최종자본5=self.L['평가금액']+self.L['현재잔액']; 최종수익5=최종자본5-초기자본5; self.D['l_profit']=round((최종수익5/초기자본5)*100,2)
    ...
    def init_value(self) :
        ...
        if '분할횟수' not in self.M :
            ...
            self.D['럭키자금']  = self.L['현재잔액'] = my.sv(SL['L0002'])
            self.L['분할배분']  = my.sf(SL['L0021'])
            self.L['대기시점']  = SL['L0022']
            self.L['진입시점']  = SL['L0023']
            self.L['각매가치']  = my.sf(SL['L0024']) # 각 차수별 매도 가치
            self.L['매입가치']  = SL['L0025']
        ...
        self.R['잔액이동'] = self.S['잔액이동'] = self.V['잔액이동'] = self.N['잔액이동'] = self.L['잔액이동'] = False
         # -------------------------------------------------------------------------------------------
        if  self.chart : # 챠트작성
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def print_backtest(self) :
        ...
        #--------------------------------------------------------
        for tac,key,key2 in [(self.R,'기회','r'),(self.S,'안정','s'),(self.N,'생활','t'),(self.L,'럭키','l')] :
