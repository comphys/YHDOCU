...
class RSN :
    ...
    def calculate_A(self,tac,key) :
        ...
        if  tac['매도수량'] :
            ...
            tac['보유수량'] -= tac['매도수량']
            tac['현재잔액'] += tac['매도금액']
            # 그래프에서 토탈가치 표시를 위해 매도일 평가금액도 계산해 주어야 한다.
            tac['평가금액']  = 0.0
            tac['중매합계'] += tac['매도금액']
            tac['현수익률'] = (self.M['당일종가'] / tac['평균단가'] -1) * 100  if tac['평균단가'] else 0.00 
            tac['평균단가']  = 0.0
...
# -------------------------------------------------------------------------------------------------------------------------------------------     
    def new_day(self) :
        ...
        self.set_value(['예정수량','매수예가','매수금액','매수수량','매도수량','매도예가','매도금액','수익현황','현수익률','평균단가','평가금액','중익합계','중매합계'],0)
...
# -------------------------------------------------------------------------------------------------------------------------------------------   
    ...
    def simulate(self,printOut=False) :
        ...
        for idx,BD in enumerate(self.B) : 
            ...
            self.set_value(['매도수량','매도금액','매수수량','매수금액','수익현황','현수익률','실현수익','수수료등'],0)
...
# -------------------------------------------------------------------------------------------------------------------------------------------         
    def init_value(self) :
        ...
        if '분할횟수' not in self.M :
            ...
            if '일반자금' not in self.D : self.D['일반자금']  = '60,000'
...
# -------------------------------------------------------------------------------------------------------------------------------------------            
    ...
    def get_simulLog(self,tactic) :
        ...
        if   tactic == 'R' : tac = self.R; key = '기회'; 초기자금 = my.sv(self.D['기회자금'])
        ...
        LD['sub18'] = f"{self.N['매금단계'][self.N['매수차수']+1]:,.2f}" if tactic == 'N' else tac['기초수량'] 
