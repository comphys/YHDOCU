...
# ------------------------------------------------------------------------------------------------------------------------------------------ 
    ...
    def rebalance(self)  :
        ...
        self.M['분할자금'] = round( self.M['현재잔액'] / self.M['최대차수']) 
    ...
    def today_buy(self) :
        ...
        if  self.M['당일종가'] <= self.M['매수예가'] : 
            ...
            self.M['매수차수'] += 1
            self.M['진행상황']  = str(self.M['매수차수']) + '차 매수' if self.M['예정수량'] else ' '

    def tomorrow_buy(self) :
        
        if  self.M['매수차수'] >  self.M['최대차수']-1 : self.M['예정수량'] = 0; return
        if  self.M['매수차수'] == self.M['최대차수']-1 : self.M['분할자금'] = self.M['현재잔액']
        
        self.M['매수예가'] = round(self.M['평균단가'],2) if self.M['평균단가'] else self.M['당일종가']
        self.M['예정수량'] = int(self.M['분할자금'] / self.M['매수예가'] ) 
        
    def tomorrow_sell(self) :
        
        if not self.M['보유수량'] : return
        self.M['매도예가'] = my.round_up(self.M['평균단가'] * self.M['매도가치'])

        if  self.M['매수차수'] >  self.M['최대차수']-1 :
            self.M['매도예가'] = my.round_up(self.M['평균단가'] * self.M['탈출가치'])
    ...
    def new_day(self) :
        ...
        if  self.M['당일종가'] <=  진입단가  :
            ...
            self.M['기록시즌'] += 1
            self.M['현재날수'] = 1
            self.M['매수수량']  = int( self.M['분할자금']/진입단가 )
            ...
            self.M['매수차수'] = 1
            self.M['진행상황'] = str(self.M['매수차수']) + '차 매수'
            self.M['첫날기록'] = False
    ...
    def nextStep(self) :
        ...
        if  self.M['첫날기록'] or not self.M['보유수량'] : 
            ...
            self.D['N_생활매수가'] = self.M['당일종가']
            self.D['N_생활매수량'] = int( self.M['분할자금'] / self.D['N_생활매수가'] )
            ...
            self.D['N_생활매수가'] = f"{self.M['매수예가']:.2f}"
            self.D['N_생활평대비'] = self.next_percent(self.M['평균단가'],self.M['매수예가']) 
            self.D['N_생활종대비'] = self.next_percent(self.M['당일종가'],self.M['매수예가'])
            self.D['N_생활배분금'] = self.M['분할자금']
