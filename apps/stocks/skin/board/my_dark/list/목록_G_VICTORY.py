...
class 목록_G_VICTORY(SKIN) :
    ...
    def chart(self) :
        ...
        if chart_data :
            ...
            self.D['현재시즌'] = LD['sub1']
            self.D['시즌첫날'] = self.DB.one(f"SELECT add0 FROM {self.D['tbl']} WHERE sub1='{self.D['현재시즌']}' and sub12='1'")
            ...
            for idx,item in enumerate(self.D['LIST']) :
                ...
                for key in self.D['list_order'] :
                    ...
                    if   key == 'no'    : 
                        ...
                        if float(txt) > 0 : tx[key] = f"<td class='list-bears2'>{float(txt):,.2f}</td>"
