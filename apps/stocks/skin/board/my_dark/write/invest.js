...
function load_value() {
    if( AutoCalculated ) return;
        ...
        // SOXL
        vtc('add11',0,2);          vtc('add12',0, 2);                  vtc('add5',0,0);
...
function client_calculate() {
	...
	// 사용자에 의해 변화가 가능한 다음 수치는 화면에서 입력받게 한다.
	입금액수 = ctv('add1','f'); 	출금액수 = ctv('add2','f'); 
	매수금액 = ctv('add11','f');    매도금액 = ctv('add12','f');  	변동수량 = ctv('add5','i'); 
	마감금액 = ctv('add14','f');
...
function check_buy() {
    if( 경과일수 == 0 ) { 
        if( 마감금액 <= 매수가격) { 
			...
			vtc('add5', 기초수량,0); vtc('add11',마감금액 * 기초수량, 2);  }
    } else {
		if( 마감금액 <= 매수가격 ) { vtc('add5', 매수수량,0); vtc('add11',마감금액 * 매수수량, 2);}
	}
}
...
function check_sell() {
    if( 경과일수 == 0 ) return;
    if( 마감금액 >= 매도가격 ) {
		vtc('add5', -매도수량,0); vtc('add12', 마감금액 * 매도수량, 2);}
}
