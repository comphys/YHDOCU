...
function client_calculate() {
	...
	vtc('add17',가치합계,2);    vtc('sub25',입금합계,2);    vtc('sub26',출금합계,2);    vtc('sub7',회복전략,1);
	vtc('sub29',진행상황,-2);   vtc('sub30',수수료등,2);    vtc('sub31',누적수료,2);    vtc('sub11',양도세금,2);
...
function load_value() {
    if( AutoCalculated ) return;
        ...
        마감금액= s_load('add3','f','H');  보유수량 = s_load('add9','i');      
        평균단가= s_load('add7','f');      매도누적 = s_load('sub15','f');     매수누적 = s_load('sub14','f');  현매수금= s_load('add6','f');
        연속상승= s_load('add9','i','H');  연속하락 = s_load('add10','i','H'); 종가변동 = s_load('add8','f','H');
        // 투자전략 - 변수선언
		...
		입금합계=s_load('sub25','f');      출금합계 = s_load('sub26','f');     회복전략 = s_load('sub7','f');
        누적수료= s_load('sub31','f');     보존금액 = s_load('sub32','f');     양도세금 = s_load('sub11','f');
        ...
        vtc('add7',평균단가,4);     vtc('sub15',매도누적,2);            vtc('sub14',매수누적,2);    vtc('add6',현매수금,2);
        vtc('sub5',연속상승,0);     vtc('sub6',연속하락,0);             vtc('add20',종가변동,2);    
    
        // 투자전략
		vtc('sub1',현재시즌,0);     vtc('sub4',일매수금,0);
		vtc('sub12',경과일수,0);    vtc('sub18',기초수량,0); 
        vtc('sub25',입금합계,2);    vtc('sub26',출금합계,2);  vtc('sub7',회복전략,1); 
        vtc('sub31',누적수료,2);    vtc('sub11',양도세금,2);  
