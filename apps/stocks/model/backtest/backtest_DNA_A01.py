...
class M_backtest_DNA_A01(Model) :
...
# 강제매도 제거
    ...
    def print_backtest(self) :
        ...
        # 날수 계산
        self.M['날수'] += 1
        if self.M['진행상황'] in ('전량매도') : self.M['날수'] = 0 
    ...
    def calculate(self)  :
        ...
        if  self.M['진행상황'] in ('전량매도','부분매도') :
            self.M['수익현황'] = self.M['매도수익']
            self.M['수익률']   = self.M['매수익률']
            self.rebalance()      

        if self.M['진행상황'] in ('전량매도') : 
            self.M['전략매금'] = 0
            self.M['위기전략'] = False
            self.M['매도횟수'] = 0
    ...
    def new_day(self) :
        ...
        self.M['진행'] = round(self.M['총매수금'] / self.D['init_capital'] * 100,1)


        # 매수 두번째 : 6회차 이하 구입전략
    def base_buy(self,opt)  :
        ...
        if opt == 'AM' :
            ...
            if  self.M['당일종가'] <= 큰단가금액 : 
                self.M['체결수량'] += math.ceil(매수금액2 / 큰단가금액) ; self.M['회차'] += 0.5 ; self.M['구매코드'] = 'II'        
            if  self.M['당일종가'] <= 평단가금액 : 
                self.M['체결수량'] += math.ceil(매수금액1 / 평단가금액) ; self.M['회차'] += 0.5 ; self.M['구매코드'] = 'I'
            self.M['진행상황'] = '일반매수'
    ...
    def normal_sell(self) :
        ...
        if self.M['당일종가'] >= 매도가격1 : self.M['매도금액'] += self.M['당일종가'] * 매도수량1  ; 매도수량 += 매도수량1  
        if self.M['당일종가'] >= 매도가격2 : self.M['매도금액'] += self.M['당일종가'] * 매도수량2  ; 매도수량 += 매도수량2  
    ...
    def mdd_sell(self,opt) :

        매도가격 = round(self.M['평균단가'] * (1+ opt/100 ),3) 
        매도수량 = self.M['보유수량'] 
    ...
    def acc_old(self) :
        ...
        # buy_price = min(self.M['전일종가'],self.M['평균단가'])
    ...
    def test_it(self) :
        ...
        #   매수부분 --------------------------------------------------------------------------------------------------
            ...
            if    self.M['진행'] < 40 : self.base_buy('AM')
            elif  self.M['진행'] < 70 : self.base_buy('PM')
            elif  self.M['진행'] <100 : self.mdd_buy(5)
            elif  self.M['진행'] <120 : self.mdd_buy(10)
            elif  self.M['진행'] <140 : self.mdd_buy(15)
