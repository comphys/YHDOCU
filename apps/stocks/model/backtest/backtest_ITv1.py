...
# 무한매수법의 개인 변형 적용으로 평균가 다운 전략
    ...
    def calculate(self)  :
        ...
        self.M['매도금액']  =  self.M['매도수량'] * self.M['매도단가']
        self.M['보유수량'] +=  self.M['매수수량'] - self.M['매도수량']
        self.M['평가금액']  =  self.M['당일종가'] * self.M['보유수량']

        self.M['총매수금']  =  self.M['총매수금'] + self.M['매수금액'] if not self.M['매도수량'] else self.M['매수금액']
        self.M['평균단가']  =  self.M['총매수금'] / self.M['보유수량'] if self.M['보유수량'] != 0 else 0
        self.M['가용잔액']  =  self.M['가용잔액'] - self.M['매수금액'] + self.M['매도금액']

        self.M['수익현황']  =  self.M['평가금액'] - self.M['총매수금']
        self.M['수익률']    = (self.M['수익현황'] / self.M['총매수금']) * 100 if self.M['총매수금'] else 0

        if  self.M['매도수량'] and self.M['리밸런싱'] : self.rebalance()   
        ...
        if not self.M['평균단가'] : self.M['첫날기록'] = True
    ...
    def normal_buy(self) :
        ...
        if  self.M['가용잔액'] <= self.M['일매수금'] : 
            return

        if  self.M['당일종가'] <= self.M['평균단가'] * self.M['큰단가치'] : 
            self.M['매수수량'] += math.ceil(매수금액2 / 큰단가금액) 
            self.M['거래코드'] = 'B'  

        if  self.M['당일종가'] <= 평단가금액 : 
            self.M['매수수량'] += math.ceil(매수금액1 / 평단가금액)  
            self.M['거래코드'] = 'A'
