...
# 분할 매수를 통한 수익 극대화 전략
    ...
    def buy_step(self)   :
        ...
        if self.M['매수횟수'] == 1 : 매수수량 = self.M['기초수량'] 
        elif self.M['매수횟수'] == 2 : 매수수량 = math.ceil(self.M['기초수량'] * 1.5 )
        else : 매수수량 = 0
    ...
    def new_day(self) :
        ...
        self.M['매수수량']  = math.ceil(self.M['일매수금']*3/self.M['전일종가'])
    ...
    def normal_sell(self) :
        ...
        if self.M['날수'] + 1 >= self.M['강매시작'] : 매도가격 = self.M['평균단가'] * 0.9
