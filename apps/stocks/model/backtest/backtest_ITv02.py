...
class M_backtest_ITv02(Model) :

# 무한매수법 V2을 활용한 백테스트에 적용
# 무한매수법 공식 룰을 따른 것으로, 분활횟수, 강제매도 조건 외 기타 추가사항은 반영되지 않는다. 
    ...
    def normal_buy(self) :
        if self.M['회차'] < int(self.M['분할횟수'] / 2) :
            ...
            if self.M['당일종가'] <= 평단가매수 : self.M['체결수량'] += math.ceil(매수금액1 / 평단가매수) ; self.M['회차'] += 1.0 ; self.M['구매코드'] += 'M'
    ...
    def test_it(self) :
        ...
        #   매수부분 --------------------------------------------------------------------------------------------------
            # step3 : 정상매수
            if self.M['가용잔액'] > 0 and self.M['회차'] <= self.M['분할횟수'] : self.normal_buy()
