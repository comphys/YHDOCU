...
# 분할 매수를 통한 수익 극대화 전략
    ...
    def test_it(self) :
        ...
        for idx,BD in enumerate(self.B) :
            ...
            self.M['당일전속상승'] = int(self.B[idx-1]['add9'])
    ...
    def get_start(self) :
        ...
        self.DB.tbl, self.DB.wre, self.DB.odr = ('h_stockHistory_board',f"add1='SOXX' AND add0 BETWEEN '{old_date}' AND '{self.D['end_date']}'",'add0')
        self.B1 = self.DB.get('add0,add3') # 날자, 종가 
