...
class M_backtest_new_idea(Model) :
...
# 무한매수법의 개인 변형 적용으로 평균가 다운 전략
    ...
    def strategy_buy(self) :
        ...
        매수금액 = self.M['평균단가'] * 1.0
        if self.M['당일종가'] <= 매수금액 : 
            
            한도금액  = self.M['추가자본'] + self.M['가용잔액']
            기본수량  = math.ceil(self.M['일매수금'] / 매수금액)
        
            if  한도금액 < self.M['일매수금']  :
                self.M['매수수량']  = int(한도금액 / 매수금액)
                self.M['위기전략'] = True
            else : self.M['매수수량']  = 기본수량 
    ...
    def test_it(self) :
        ...
        for idx,BD in enumerate(self.B) :
            ...
            if self.M['위기전략'] and self.M['수량확보'] : self.strategy_sell()
                ...
                if self.M['진행'] < CP : self.base_buy() 
                else :
                    if self.M['전략매금'] : self.strategy_buy()
