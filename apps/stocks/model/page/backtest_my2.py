...
class M_backtest_my2(Model) :

# 무한매수법 V1을 활용한 백테스트에 적용
# 수수료는 계산하지 않으며, 매도발생 시 가용잔액게 합산하여 복리로 계산한다.
    ...
    def test_it(self) :
        ...
        for idx,BD in enumerate(self.B) : 
            ...
            if idx == 0 or 첫날기록 : #첫날 기록으로 첫날은 종가로 구입되는 것으로 가정
                ...
                진행상황 = '정상진행'
                체결수량,체결수량1,체결수량2 = (0,0,0)
                체결수량3,체결수량4,체결수량5,체결수량6,체결수량7 = (0,0,0,0,0)
                ...
                if 회차 > 40 and 당일고가 > 강제매도가 : 
                 ...
                 # 매도상황 2 정상매도,  매도는 종가에 결정되는 것이 아닌 장중최고가에 결정되는 것으로 가정
                    ...
                    if 당일고가 >= 매도가격1 : 매도금액 += 당일고가 * 매도수량1  ; 매도수량 += 매도수량1 ; 
                    if 당일고가 >= 매도가격2 : 매도금액 += 당일고가 * 매도수량2  ; 매도수량 += 매도수량2 ; 
                ...
                if 가용잔액 > 0 :
                    ...
                    if 당일종가 <= 종가매수1 : 체결수량3 = math.ceil(일매수금 / 종가매수1 ) ; 회차 += 1.0 
                    if 당일종가 <= 종가매수2 : 체결수량4 = math.ceil(일매수금 / 종가매수2 ) ; 회차 += 1.0
                    if 당일종가 <= 종가매수3 : 체결수량5 = math.ceil(일매수금 / 종가매수3 ) ; 회차 += 1.0
                    if 당일종가 <= 종가매수4 : 체결수량6 = math.ceil(일매수금 / 종가매수4 ) ; 회차 += 1.0
                    if 당일종가 <= 종가매수5 : 체결수량7 = math.ceil(일매수금 / 종가매수5 ) ; 회차 += 1.0
                    #-------------------------------------------------------------------------------------------------

                
                체결수량 = 체결수량1 + 체결수량2 + 체결수량3 + 체결수량4 + 체결수량5 + 체결수량6 + 체결수량7
                매수금액 = 체결수량 * 당일종가
