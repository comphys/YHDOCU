...
class Stock_longterm_info(Control) : 
    ...
    def get_subinfo(self) :
        ...
        elif  opt == '매수금1'  : qry_opt = 'add5,add7,add8' ; qry_wre = 'add5'
        elif  opt == '매도금1'  : qry_opt = 'add6,add7,add8' ; qry_wre = 'add6'
        elif  opt == '매수금2'  : qry_opt = 'add11,add13,add14' ; qry_wre = 'add11'
        elif  opt == '매도금2'  : qry_opt = 'add12,add13,add14' ; qry_wre = 'add12'
        ...
        if D and opt in ('입금내역','출금내역','배당내역'):
            ...
            output  = "<div style='width:300px;max-height:250px;background-color:black;overflow-x:hidden' ondblclick=\"h_dialog.close('POP_INFO')\">"
            output += "<table class='table table-bordered table-striped;' style='background-color:#0A2229;color:#e1e1e1;'>"
            output += f"<tr style='background-color:#0B2F3A'><td>일자</td><td style='text-align:right'>금액</td><td style='text-align:right'>수량</td><td style='text-align:right'>종가</td></tr>"
            for d in D :
                output += f"<tr><td>{d[0]}</td><td style='text-align:right'>{float(d[1]):,.2f}</td><td style='text-align:right'>{int(d[2])}</td><td style='text-align:right'>{float(d[3]):,.2f}</td></tr>"
            output += "</table></div>"
