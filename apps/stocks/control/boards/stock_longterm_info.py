...
class Stock_longterm_info(Control) : 
    ...
    def get_subinfo(self) :
        ...
        elif  opt == '종가1'    : qry = f"SELECT add0,add8 FROM {self.tbl} WHERE add8 = (SELECT min(add8) FROM {self.tbl}) OR add8 = (SELECT max(add8) FROM {self.tbl})"
        elif  opt == '종가2'    : qry = f"SELECT add0,add14 FROM {self.tbl} WHERE add14 = (SELECT min(add14) FROM {self.tbl}) OR add14 = (SELECT max(add14) FROM {self.tbl})"
