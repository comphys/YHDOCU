...
class Stock_daily(Control) : 
    ...
    def init_value(self) :
        ...
        self.M['체결수량'] = self.D['post']['add7']   ; self.M['체결수량'] = int(self.M['체결수량'].replace(',',''))   if self.M['체결수량'] else 0.0 
        ...
        self.M['연속하락'] = self.old_price_trace() ; self.M['위기전략'] = 'NO' ; self.M['전략매금'] = 0 ; self.M['전략가격'] = 0
    ...
    def strategy_sell(self) :
        if (self.M['평균단가'] - self.M['전일종가'])/self.M['평균단가'] < 0.12 :  self.M['진행상황'] = '기준이내' ; return
        self.M['전략수량'] = int(self.M['보유수량']*0.6) ; self.M['전략단가'] = self.M['평균단가'] * (1+self.M['매도시점'])
    ...
    def normal_sell(self) :
        ...
        self.M['둘째수량'] = self.M['보유수량'] - self.M['첫매수량']
        self.M['첫째단가'] = self.M['평균단가'] * self.M['첫매가치']
        self.M['둘째단가'] = self.M['평균단가'] * self.M['둘매가치']
    ...
    def return_value(self) :
        ...
        update['add8']   = f"{round(self.M['매수금액'],4):,.2f}"; update['add9'] = f"{round(self.M['평균단가'],4):,.2f}" ; update['add10'] = f"{self.M['보유수량']:,}"
