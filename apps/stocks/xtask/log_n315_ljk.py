...
if  skip :
    ...
    ini_data = N315.DB.oneline(f"SELECT add18,add19 FROM h_{bid}_board ORDER BY add0 DESC LIMIT 1")
    ini_date = ini_data[0]
    ...
    LS = N315.DB.last_data_one('add1',f"h_{bid}_board") # last season
    
    LD['add1'] = int(LS) + 1 if LD['add2'] == 1 else LS
        
    if  not LD['첫날기록'] or LD['add6'] in('익절매도','손절매도') :   
        del LD['첫날기록']  
        qry=N315.DB.qry_insert(f"h_{bid}_board",LD)
        N315.DB.exe(qry)
        N315.send_message(f"{today}일 {bid} 업데이트 완료")
